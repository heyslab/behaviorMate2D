...
def _settings_(msg, _time, _dict):
    ...
    if 'reward' in _dict['__trial_info']:
        ...
        reward_ids = reward.get('id', 'reward')
...
def _behavior_mate_(msg, _time, _dict):
    ...
    def _test_valve_(msg, _time, _dict):
        if 'test_valve' not in _dict.keys():
            _dict['test_valve'] = []

        _dict['test_valve'].append(_time)
        print(_time)
    ...
    for key in msg.keys():
        ...
        elif '_{}_'.format(key) in locals().keys():
...
def _x_(msg, _time, _dict):
    """
    reformat the x component for the current trial
    :param msg:  dictionary with the trial specification
    :param _time: time of trial
    :param _dict: stores current trial's formatted info
    :type msg: dict
    ...
    :rtype: void
    """
    if '__position_x' not in _dict.keys():
        _dict['__position_x'] = np.array([]).reshape((0, 2))

    _dict['__position_x'] = np.vstack(
        (_dict['__position_x'], [_time, float(msg)]))
...
def parseFile(filepath):
    ...
    if "__position_x" in res.keys():
        res['__position_x'][:, 0] -= offset
        res['__position_x'] = res['__position_x'][np.where(
            res['__position_x'][:, 0] >= 0)]
        if res['__position_x'][0, 0] != 0:
            res['__position_x'] = np.insert(
                res['__position_x'], 0, [0, res['__position_x'][0, 1]], axis=0)

    if '__position_x' in res.keys() and '__position_y' in res.keys():
        position = pd.concat((
            pd.DataFrame(res['__position_x']).set_index(0),
            pd.DataFrame(res['__position_y']).set_index(0)), axis=1).reset_index().values
